  ...
  return (
    <div className="flex">
      ...
      <div className="w-screen h-screen md:w-[60vw] px-12 pt-8 pb-12">
        <h2 className="text-lg font-medium text-black">Expense Tracker</h2> {/* corrected text-log */}
        {children}
      </div>
      ...
      <div className="hidden md:block w-[40vw] h-screen bg-violet-100 bg-auth-bg-img bg-cover bg-no-repeat bg-center overflow-hidden p-8 relative">
        ...
        <img 
          src={CARD2}
          className=' max-w-[350px] absolute bottom-10 shadow-lg shadow-blue-400/15 rounded-[10px]'/> {/*h-[50%]*/}
      </div>
...
const StatsInfoCard = ({ icon, label, value, color }) => {
  return (
    <div className="flex gap-6 bg-white p-4 rounded-xl shadow-md shadow-purple-400/10 border border-gray-200/50 z-10">
      ...
      <div
        className='w-12 h-12 flex items-center justify-center text-[26px] text-white ${color} rounded-full drop-shadow-xl' // corrected backticks
      >
        {icon}
      </div>

      <div className="mt-2 text-left"> {/* fixed text alignment */}
